...
const FEATURES = [
  ...
  { icon: Sparkles, title: "Birth Chart Profile", desc: "A unique personality map blending your chart with who you truly are." },
  { icon: Eye, title: "Why This Insight?", desc: "Transparent reasoning behind every astrology insight." },
  { icon: Star, title: "Curiosity Cards", desc: "Hidden strengths, soul lessons, and 90-day outlooks." },
  { icon: Zap, title: "Daily Astrology Insights", desc: "Personalized daily messages that evolve with your journey." },
  { icon: Heart, title: "Session Memory", desc: "Your guide remembers you — every return feels like coming home." },
];
    ...
    <section className="relative z-10 px-4 sm:px-6 py-16 sm:py-28 max-w-7xl mx-auto w-full">
      <FadeIn>
        <h2 className="font-display text-3xl sm:text-4xl text-center text-white/90 mb-3">
          Not a horoscope. A personal astrology guide.
        </h2>
...
const TESTIMONIALS = [
  ...
  { name: "Priya, 27", sign: "Pisces", text: "The daily astrology messages feel personal — not generic horoscope fluff. I open it every morning now." },
];
...
export function Testimonials() {
  return (
    <section className="relative z-10 px-4 sm:px-6 py-12 sm:py-20 max-w-5xl mx-auto w-full">
      <FadeIn>
        <p className="text-xs tracking-[0.3em] uppercase text-white/25 text-center mb-10">Trusted by astrology seekers</p>
      </FadeIn>
      <div className="grid md:grid-cols-3 gap-5">
        {TESTIMONIALS.map((t, i) => {
          const tokens = getElementTokens(t.sign);
          return (
          <FadeIn key={t.name} delay={i * 0.1}>
            <GlassCard padding="sm" className="h-full relative overflow-hidden">
              <p className="text-sm text-white/40 leading-relaxed italic mb-4">&ldquo;{t.text}&rdquo;</p>
              <div className="flex items-center gap-3">
                <div
                  className="w-10 h-10 rounded-full bg-white/[0.04] border flex items-center justify-center"
                  style={{ borderColor: tokens.muted }}
                >
                  <ZodiacSignImage sign={t.sign} size={28} ring={false} shimmer />
                </div>
                <div>
